...
MY_FILE_ADAPTER = SimpleJSONFileAdapter(".sdk-manage-flow.json")
...
def _get_scopes():
    flow_scopes = [globus_sdk.FlowsClient.scopes.manage_flows]
    group_scopes = [
        globus_sdk.GroupsClient.scopes.view_my_groups_and_memberships,
        "urn:globus:auth:scope:groups.api.globus.org:all",
    ]
    return flow_scopes + group_scopes
...
def get_authorizer(kind: str, *, reset_tokens: bool = False):
    ...
    if tokens is None:
        raise ValueError(f"Failed to get tokens for {kind}")
...
def get_flow_client(reset_tokens: bool = False):
    authorizer = get_authorizer("flow", reset_tokens=reset_tokens)
